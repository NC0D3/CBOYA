...
class LiveViewWorker(QThread):
    # Señales para enviar los frames procesados al hilo principal
    frame_ready = pyqtSignal(QPixmap, QPixmap)  # Señal con dos imágenes: normal y binarizada

    def __init__(self, picam, threshold, parent=None):
        super().__init__(parent)
        self.picam = picam
        self.threshold = threshold
        self.running = True  # Controla si el hilo sigue ejecutándose
        ...
        while self.running:
            try:
                ...
                if frame is not None and frame.ndim==3:
                    ...
                    painter.end()
                    # Emito las imágenes procesadas al hilo principal
                    self.frame_ready.emit(rounded, rounded2)
            except Exception as e:
                print(f"Error en LiveViewWorker: {e}")
                self.running = False
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        #temporizador
        # Inicializa el hilo de LiveView
        self.liveview_worker = LiveViewWorker(self.picam, self.threshold)
        self.liveview_worker.frame_ready.connect(self.LiveView)  # Conecta la señal al método de actualización

        # Comienza el hilo de LiveView
        self.liveview_worker.start()
    ...
    def LiveView(self, rounded, rounded2):
